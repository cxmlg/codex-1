...
def scan_file(path: Path, root: Path, stats: ScanStats | None = None) -> list[Item]:
    try:
    ...
    for macro, out_kind, tags, decl_kind in macro_rules:
        for _start, end, specs in collect_macro_invocations(lines, macro):
            ...
            if stats is not None:
                stats.add_item(out_kind)

    return items


def print_progress(current: int, total: int, file: Path) -> None:
    print(f"[scan {current}/{total}] {file}", file=sys.stderr)


def print_stats(stats: ScanStats) -> None:
    print("[stats] 扫描完成", file=sys.stderr)
    print(
        f"[stats] 文件: total={stats.files_total}, scanned={stats.files_scanned}, failed={stats.files_failed}",
        file=sys.stderr,
    )
    print(
        "[stats] 命中: "
        f"class={stats.class_count}, interface={stats.interface_count}, struct={stats.struct_count}, "
        f"enum={stats.enum_count}, property={stats.property_count}, function={stats.function_count}, "
        f"total={stats.item_total}",
        file=sys.stderr,
    )
...
def main() -> int:
    ...
    parser.add_argument("--no-progress", action="store_true", help="Disable per-file scan progress output")
    parser.add_argument("--no-stats", action="store_true", help="Disable scan statistics output")
    args = parser.parse_args()

    root = Path(args.root).resolve()
    files = list(iter_code_files(root))
    stats = ScanStats(files_total=len(files))
    items: list[Item] = []
    for idx, file in enumerate(files, start=1):
        if not args.no_progress:
            print_progress(idx, len(files), file)
        stats.files_scanned += 1
        items.extend(scan_file(file, root, stats=stats))
